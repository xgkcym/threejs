uniform vec2 waveNum;
uniform float waveHeight;
uniform float uTime;
uniform float uTimeSpeed;
uniform float smallWaveNum;
uniform float smallWaveSpeed;
uniform float smallWaveSize;
uniform float smallWaveHeight;
uniform float smallWaveMultiplier;
varying float vElevation;


const float max_its = 100.0;
...
void main(){
    ...
    float elevation=sin(modelPosition.x*waveNum.x+uTime*uTimeSpeed)*
    sin(modelPosition.z*waveNum.y+uTime*uTimeSpeed)*
    waveHeight;
    for(float i=1.0;i<=max_its;i++){
        if(i>smallWaveSize){
            break;
        }
        elevation-=abs(cnoise(vec3(modelPosition.xz*i*smallWaveNum,uTime*smallWaveSpeed))*smallWaveHeight/i);
    }
